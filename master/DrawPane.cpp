...
LoadSceneResult BasicDrawPane::loadScene()
{
	...
	while (fgets(line, sizeof(line), sceneFile))
	{
		...
		void* destinations[] = {
				...
				&shape_id_bank[num_objects],
				&material_id_bank[num_objects],
				&object_position_bank[num_objects * 3 + 0],
				&object_position_bank[num_objects * 3 + 1],
				&object_position_bank[num_objects * 3 + 2],
				&object_right_bank[num_objects * 3 + 0],
				&object_right_bank[num_objects * 3 + 1],
				&object_right_bank[num_objects * 3 + 2],
				&object_up_bank[num_objects * 3 + 0],
				&object_up_bank[num_objects * 3 + 1],
				&object_up_bank[num_objects * 3 + 2],
				&object_forward_bank[num_objects * 3 + 0],
				&object_forward_bank[num_objects * 3 + 1],
				&object_forward_bank[num_objects * 3 + 2]
		};
		...
		while (destination_index < 14) {

			for (int endpoint = startpoint; endpoint < 1024; endpoint++) {

				if (line[endpoint] == ' ' || line[endpoint] == '\n' || line[endpoint] == '\r') {


					//strncpy does not insert null terminator itself
					strncpy(substring, &line[startpoint], (size_t)(endpoint - startpoint)); //endpoint not included
					substring[endpoint - startpoint] = '\0';


					sscanf(substring, formats[destination_index], destinations[destination_index]);
	...
	FILE* buildProcedureFile = fopen("buildprocedure.txt", "rb");

	while (fgets(line, sizeof(line), buildProcedureFile)) {

		sscanf(line, "%d %d %d %d",

			&build_procedure_data[num_build_steps * 4 + 0],
			&build_procedure_data[num_build_steps * 4 + 1],
			&build_procedure_data[num_build_steps * 4 + 2],
			&build_procedure_data[num_build_steps * 4 + 3]
